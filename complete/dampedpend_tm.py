...
@jax.jit
def combined_loss(model, activations, pst_x, pst_y, lpst_x, lpst_y, t3, t4, alpha=1.0, beta=1.0):
    ...
    return alpha*mse_loss(lterm_x, rterm_x) + beta*mse_loss(lterm_y, rterm_y)

@jax.jit
def adam_update_gd(model, adam_state, activations, pst_x, pst_y, lpst_x, lpst_y, t3, t4, lr, alpha=1.0, beta=1.0):
    """ Update the parameters of the model using the Adam optimizer on the combined loss function """
    t = adam_state["t"] + 1

    total_loss_grads = jax.grad(combined_loss)(model, activations, pst_x, pst_y, lpst_x, lpst_y, t3, t4, alpha, beta)
...
def get_ode_functions(num_derivatives=3):
    ...
    x, y, b, L = sp.symbols('x y b L')
    params, state_vars = [b, L], [x, y]
    pendEqn = [ y, -b*y - (9.81/L)*sp.sin(x) ]

    F_dot = sp.Matrix([pendEqn[i] for i in range(len(pendEqn))])
    F_ddot = sp.Matrix([sp.diff(F_dot[i], x) * pendEqn[0] + sp.diff(F_dot[i], y) * pendEqn[1] for i in range(len(F_dot))])
    F_dddot = sp.Matrix([sp.diff(F_ddot[i], x) * pendEqn[0] + sp.diff(F_ddot[i], y) * pendEqn[1] for i in range(len(F_ddot))])
...
def define_ode(init_conds, t_eval, duration):
    """ Create a dataset for training the model """
    x, y, b, L = sp.symbols('x y b L')
    diffEq = [ y, -b*y - (9.81/L)*sp.sin(x) ]
    diffEq_func = [ sp.lambdify((x, y, b, L), e) for e in diffEq ]

    def Eqn(t, y, b, L):
        x, y = y
        dydt = [ diffEq_func[i](x, y, b, L) for i in range(len(diffEq_func))]
        return dydt
    
    solution_set = []
    for _, x0, y0, b0, L0 in init_conds:
        sol = solve_ivp(Eqn, t_span=[0, duration], t_eval=t_eval, y0=[x0, y0], args=(b0, L0), method='RK45')
...
def sample_generation(s_size, steps, t_eval, key):
    ...
    b_sample = jax.random.uniform(get_random_key(key+9), minval=0.01, maxval=0.5, shape=(s_size, ))
    L_sample = jax.random.uniform(get_random_key(key+999), minval=1, maxval=10, shape=(s_size, ))

    dataset = jnp.stack([x_sample, y_sample, b_sample, L_sample], axis=1)
...
def return_func_output(eqn_num, state, func):
    return jax.vmap(lambda x, y, b, L: func[eqn_num](x, y, b, L))(state[:, 0], state[:, 1], state[:, 2], state[:, 3])
...
def get_intr_time_coupled_sums(activations, steps, t, t2, t3):
    ...
    return ps_tc_x, ps_tc_y, lps_tc_x, lps_tc_y
...
def create(plotpred: bool, savepred: bool, savemetrics: bool, name: str):
    ...
    for iter in range(1):
        ...
        adam_state = initialize_adam_state(model, num_var=2)  # Initialize Adam state
        batch_counter = 0
        epochs = 500
        lr = 5e-2
        ...
        for itrain in range(batch_counter, batch_counter+epochs):
            ...
            ps_tc_x, ps_tc_y, lps_tc_x, lps_tc_y = get_intr_time_coupled_sums(activations, steps, train_t, train_t2, train_t3)
            
            model, adam_state = adam_update_gd(model, adam_state, activations, ps_tc_x, ps_tc_y, lps_tc_x, lps_tc_y, train_t3, train_t4, lr, alpha=1.0, beta=1.0)
            ...
            if itrain % 100 == 0:
                ...
                ps_tc_x, ps_tc_y, _, _ = get_intr_time_coupled_sums(validation_activations, steps, val_t, val_t2, val_t3)
                ...
                print(f"Batch {itrain}:\nValidation -> MSE: {((mse_lossx + mse_lossy)/2):.5f}\tMAE: {((mae_lossx + mae_lossy)/2):.5f}\tRMSE: {((rmse_lossx + rmse_lossy)/2):.5f}")

                log_entry = (f"\nBatch {itrain}:\nMSE = {mse_lossx:.5f}, {mse_lossy:.5f}\tMAE = {mae_lossx:.5f}, {mae_lossy:.5f}\tRMSE = {rmse_lossx:.5f}, {rmse_lossy:.5f}")
                logs += log_entry
    ...
    ps_tc_x, ps_tc_y, _, _ = get_intr_time_coupled_sums(test_dataset, steps, test_t, test_t2, test_t3)
    ...
    pred_batch = np.stack([time_coupled_x, time_coupled_y], axis=1)
    ...
    if save_predictions:
        ...
        ihw, ihb = np.array(model[0][0][0]), np.array(model[0][0][1])
        how, hob = np.array(model[0][1][0]), np.array(model[0][1][1])
        model1 = {"input_hidden_weights": ihw.tolist(), "input_hidden_biases": ihb.tolist(), "hidden_output_weights": how.tolist(), "hidden_output_biases": hob.tolist()}
        with open(f"preds/{save_name}/{save_name}_model1.json", "w") as json_file:
            json.dump(model1, json_file)
        
        ihw, ihb = np.array(model[1][0][0]), np.array(model[1][0][1])
        how, hob = np.array(model[1][1][0]), np.array(model[1][1][1])
        model2 = {"input_hidden_weights": ihw.tolist(), "input_hidden_biases": ihb.tolist(), "hidden_output_weights": how.tolist(), "hidden_output_biases": hob.tolist()}
        with open(f"preds/{save_name}/{save_name}_model2.json", "w") as json_file:
            json.dump(model2, json_file)

    if plot_prediction:
        hp.plot_creations_2d(t_eval, test_dataset, test_target, steps, name=f"{save_name}_tb", no_of_params=2, no_of_svariables=2, no_plot_figures=10, set=save_name, pred_batch=pred_batch)
        hp.plot_error_distributions(pred_batch, test_target, save_name,  name=f"{save_name}_tb", no_of_svariables=2)
    ...
    for v in sec:
        ...
        if plot_prediction:
            hp.plot_creations_2d(t[:s], td, tt, s, name=f"{save_name}_tb" + "_" + str(v) + "sec", no_of_params=2, no_of_svariables=2, no_plot_figures=10, set=save_name, pred_batch=pt)
            hp.plot_error_distributions(pt, tt, save_name,  name=f"{save_name}_tb" + "_" + str(v) + "sec", no_of_svariables=2)
