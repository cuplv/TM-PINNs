...
def get_ode_functions(num_derivatives=3):
    ...
    x, y, d = sp.symbols('x y \delta')
    params, state_vars = [d], [x, y]
    pendEqn = [ y, x - x**3 - d*y ]
    ...
    # timeDerivativeEqns = [ x - x**3 - d*y, y - 3*x**2*y - d * (x - x**3 - d*y)]
...
def define_ode(init_conds, t_eval, duration):
    """ Create a dataset for training the model """
    x, y, delta = sp.symbols('x y delta')
    diffEq = [ y, x - x**3 - delta*y ]
    diffEq_func = [ sp.lambdify((x, y, delta), e) for e in diffEq ]

    def Eqn(t, y, delta):
        x, y = y
        dydt = [ diffEq_func[i](x, y, delta) for i in range(len(diffEq_func))]
        return dydt
    
    solution_set = []
    for _, x0, y0, delta0 in init_conds:
        sol = solve_ivp(Eqn, t_span=[0, duration], t_eval=t_eval, y0=[x0, y0], args=(delta0,), method='RK45')
...
def sample_generation(s_size, steps, t_eval, key):
    ...
    x_sample = jax.random.uniform(get_random_key(key), minval=-0.5, maxval=0.5, shape=(s_size, ))
    y_sample = jax.random.uniform(get_random_key(key+99), minval=-0.5, maxval=0.5, shape=(s_size, ))
    d_sample = jax.random.uniform(get_random_key(key+9), minval=0.1, maxval=0.5, shape=(s_size, ))

    dataset = jnp.stack([x_sample, y_sample, d_sample], axis=1)
...
def create(plotpred: bool, savepred: bool, savemetrics: bool, name: str):
    ...
    if plot_prediction:
        hp.plot_creations_2d(t_eval, test_dataset, test_target, steps, name=f"{save_name}_tb", no_of_params=1, no_of_svariables=2, no_plot_figures=10, set=save_name, pred_batch=pred_batch)
    ...
    for v in sec:
        ...
        if plot_prediction:
            hp.plot_creations_2d(t[:s], td, tt, s, name=f"{save_name}_tb" + "_" + str(v) + "sec", no_of_params=1, no_of_svariables=2, no_plot_figures=10, set=save_name, pred_batch=pt)
