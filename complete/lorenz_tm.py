...
def get_ode_functions(num_derivatives=3):
    ...
    x, y, z, beta, sigma, rho = sp.symbols('x y z beta sigma rho')
    params, state_vars = [beta, sigma, rho], [x, y, z]
    pendEqn = [ sigma * (y - x), x * (rho - z) - y, x * y - beta * z ]
...
def define_ode(init_conds, t_eval, duration):
    """ Create a dataset for training the model """
    x, y, z, beta, sigma, rho = sp.symbols('x y z beta sigma rho')
    diffEq = [ sigma * (y - x), x * (rho - z) - y, x * y - beta * z ]
    diffEq_func = [ sp.lambdify((x, y, z, beta, sigma, rho), e) for e in diffEq ]

    def Eqn(t, y, beta, sigma, rho):
        x, y, z = y
        dydt = [ diffEq_func[i](x, y, z, beta, sigma, rho) for i in range(len(diffEq_func))]
        return dydt
    
    solution_set = []
    for _, x0, y0, z0, beta0, sigma0, rho0 in init_conds:
        sol = solve_ivp(Eqn, t_span=[0, duration], t_eval=t_eval, y0=[x0, y0, z0], args=(beta0, sigma0, rho0), method='RK45')
...
def sample_generation(s_size, steps, t_eval, key):
    ...
    sigma_sample = jax.random.uniform(get_random_key(key+8), minval=0, maxval=1, shape=(s_size, ))
    rho_sample = jax.random.uniform(get_random_key(key+12), minval=0, maxval=1, shape=(s_size, ))

    dataset = jnp.stack([x_sample, y_sample, z_sample, beta_sample, sigma_sample, rho_sample], axis=1)
