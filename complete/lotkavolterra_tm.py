...
def get_ode_functions(num_derivatives=3):
    ...
    x, y, a, b, c, d = sp.symbols('x y alpha beta gamma delta')
    params, state_vars = [a, b, c, d], [x, y]
    pendEqn = [ a*x - b*x*y, -c*y + d*x*y ]
...
def define_ode(init_conds, t_eval, duration):
    """ Create a dataset for training the model """
    x, y, a, b, c, d = sp.symbols('x y alpha beta gamma delta')
    diffEq = [ a*x - b*x*y, -c*y + d*x*y ]
    diffEq_func = [ sp.lambdify((x, y, a, b, c, d), e) for e in diffEq ]

    def Eqn(t, y, a, b, c, d):
        x, y = y
        dydt = [ diffEq_func[i](x, y, a, b, c, d) for i in range(len(diffEq_func))]
        return dydt
    
    solution_set = []
    for _, x0, y0, a0, b0, c0, d0 in init_conds:
        sol = solve_ivp(Eqn, t_span=[0, duration], t_eval=t_eval, y0=[x0, y0], args=(a0, b0, c0, d0), method='RK45')
...
def sample_generation(s_size, steps, t_eval, key):
    ...
    a_sample = jax.random.uniform(get_random_key(key+777), minval=0.6, maxval=1.0, shape=(s_size, ))
    b_sample = jax.random.uniform(get_random_key(key+9), minval=0.2, maxval=0.5, shape=(s_size, ))
    c_sample = jax.random.uniform(get_random_key(key+7), minval=0.5, maxval=1.0, shape=(s_size, ))
    d_sample = jax.random.uniform(get_random_key(key+77), minval=0.1, maxval=0.4, shape=(s_size, ))

    dataset = jnp.stack([x_sample, y_sample, a_sample, b_sample, c_sample, d_sample], axis=1)
...
def return_func_output(eqn_num, state, func):
    return jax.vmap(lambda x, y, a, b, c, d: func[eqn_num](x, y, a, b, c, d))(state[:, 0], state[:, 1], state[:, 2], state[:, 3], state[:, 4], state[:, 5])
...
def create(plotpred: bool, savepred: bool, savemetrics: bool, name: str):
    ...
    if plot_prediction:
        hp.plot_creations_2d(t_eval, test_dataset, test_target, steps, name=f"{save_name}_tb", no_of_params=4, no_of_svariables=2, no_plot_figures=10, set=save_name, pred_batch=pred_batch)
    ...
    for v in sec:
        ...
        if plot_prediction:
            hp.plot_creations_2d(t[:s], td, tt, s, name=f"{save_name}_tb" + "_" + str(v) + "sec", no_of_params=4, no_of_svariables=2, no_plot_figures=10, set=save_name, pred_batch=pt)
