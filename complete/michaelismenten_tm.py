...
def get_ode_functions(num_derivatives=3):
    ...
    x1, x2, x3, x4, x5, x6, V1, V2, V3, V4, V5, V6 = sp.symbols('x1 x2 x3 x4 x5 x6 V1 V2 V3 V4 V5 V6')
    params = [V1, V2, V3, V4, V5, V6]
    state_vars = [x1, x2, x3, x4, x5, x6]
    eqns = [
        (V1/(0.5 + 1) - 0.1*x1),
        ((V2*x1)/(0.5 + x1) - 0.1*x2),
        ((V3*x2)/(0.5 + x2) - 0.1*x3),
    ...
    F_ddot = sp.Matrix([
        sum([sp.diff(F_dot[v], state_vars[i]) * eqns[i] for i in range(6)]) for v in range(6)
    ])
    F_dddot = sp.Matrix([
        sum([sp.diff(F_ddot[v], state_vars[i]) * eqns[i] for i in range(6)]) for v in range(6)
    ])
...
def define_ode(init_conds, t_eval, duration):
    ...
    diffEq_func = [sp.lambdify((x1, x2, x3, x4, x5, x6, V1, V2, V3, V4, V5, V6), e) for e in eqns]

    def Eqn(t, y, V1, V2, V3, V4, V5, V6):
        x1, x2, x3, x4, x5, x6 = y
        return [f(x1, x2, x3, x4, x5, x6, V1, V2, V3, V4, V5, V6) for f in diffEq_func]

    solution_set = []
    for _, x10, x20, x30, x40, x50, x60, V10, V20, V30, V40, V50, V60 in init_conds:
        sol = solve_ivp(Eqn, t_span=[0, duration], t_eval=t_eval, y0=[x10, x20, x30, x40, x50, x60], args=(V10, V20, V30, V40, V50, V60), method='RK45')
...
def sample_generation(s_size, steps, t_eval, key):
    ...
    # State variables (initial conditions range: 0.1 to 0.5)
    x1 = jax.random.uniform(get_random_key(key), minval=0.1, maxval=0.5, shape=(s_size,))
    x2 = jax.random.uniform(get_random_key(key+1), minval=0.1, maxval=0.5, shape=(s_size,))
    x3 = jax.random.uniform(get_random_key(key+2), minval=0.1, maxval=0.5, shape=(s_size,))
    x4 = jax.random.uniform(get_random_key(key+3), minval=0.1, maxval=0.5, shape=(s_size,))
    x5 = jax.random.uniform(get_random_key(key+4), minval=0.1, maxval=0.5, shape=(s_size,))
    x6 = jax.random.uniform(get_random_key(key+5), minval=0.1, maxval=0.5, shape=(s_size,))
    # Parameters (range: 0.5 to 1.0)
    V1 = jax.random.uniform(get_random_key(key+6), minval=0.5, maxval=1.0, shape=(s_size,))
    V2 = jax.random.uniform(get_random_key(key+7), minval=0.5, maxval=1.0, shape=(s_size,))
    V3 = jax.random.uniform(get_random_key(key+8), minval=0.5, maxval=1.0, shape=(s_size,))
    V4 = jax.random.uniform(get_random_key(key+9), minval=0.5, maxval=1.0, shape=(s_size,))
    V5 = jax.random.uniform(get_random_key(key+10), minval=0.5, maxval=1.0, shape=(s_size,))
    V6 = jax.random.uniform(get_random_key(key+11), minval=0.5, maxval=1.0, shape=(s_size,))
    dataset = jnp.stack([x1, x2, x3, x4, x5, x6, V1, V2, V3, V4, V5, V6], axis=1)
...
def return_func_output(eqn_num, state, func):
    # For 6D Michaelis-Menten cascade: x1,x2,x3,x4,x5,x6,V1,V2,V3,V4,V5,V6
    return jax.vmap(lambda x1, x2, x3, x4, x5, x6, V1, V2, V3, V4, V5, V6: func[eqn_num](x1, x2, x3, x4, x5, x6, V1, V2, V3, V4, V5, V6))(
...
def create(plotpred: bool, savepred: bool, savemetrics: bool, name: str):
    ...
    num_features, num_targets = 13, 1  # 1 time + 6 state + 6 param
    ...
    hp = Helpers()
    # 6 models for 6 state variables
    model_x1 = return_nnmodel(hp, ran_key, num_features, num_targets, num_layers, num_neurons_per_layer)
    model_x2 = return_nnmodel(hp, ran_key, num_features, num_targets, num_layers, num_neurons_per_layer)
    model_x3 = return_nnmodel(hp, ran_key, num_features, num_targets, num_layers, num_neurons_per_layer)
    model_x4 = return_nnmodel(hp, ran_key, num_features, num_targets, num_layers, num_neurons_per_layer)
    model_x5 = return_nnmodel(hp, ran_key, num_features, num_targets, num_layers, num_neurons_per_layer)
    model_x6 = return_nnmodel(hp, ran_key, num_features, num_targets, num_layers, num_neurons_per_layer)

    model = [model_x1, model_x2, model_x3, model_x4, model_x5, model_x6]
    ...
    test_predictions = [jnp.ravel(batched_prediction(model[i], test_dataset, silu)) for i in range(6)]
    ...
    if plot_prediction:
        hp.plot_creations_2d(t_eval, test_dataset, test_target, steps, name=f"{save_name}_tb", no_of_params=6, no_of_svariables=6, no_plot_figures=10, set=save_name, pred_batch=pred_batch)
        hp.plot_error_distributions(pred_batch, test_target, save_name,  name=f"{save_name}_tb", no_of_svariables=6)
    ...
    for v in sec:
        ...
        if plot_prediction:
            hp.plot_creations_2d(t[:s], td, tt, s, name=f"{save_name}_tb" + "_" + str(v) + "sec", no_of_params=6, no_of_svariables=6, no_plot_figures=10, set=save_name, pred_batch=pt)
            hp.plot_error_distributions(pt, tt, save_name,  name=f"{save_name}_tb" + "_" + str(v) + "sec", no_of_svariables=6)
