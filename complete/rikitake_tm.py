...
def get_ode_functions(num_derivatives=3):
    ...
    x, y, z, mu, h = sp.symbols('x y z \mu h')
    params, state_vars = [mu, h], [x, y, z]
    pendEqn = [ -mu * x + y * z, -mu * y + (z - h) * x, 1 - x * y ]
...
def define_ode(init_conds, t_eval, duration):
    """ Create a dataset for training the model """
    x, y, z, mu, h = sp.symbols('x y z \mu h')
    diffEq = [ -mu * x + y * z, -mu * y + (z - h) * x, 1 - x * y ]
    diffEq_func = [ sp.lambdify((x, y, z, mu, h), e) for e in diffEq ]

    def Eqn(t, y, mu, h):
        x, y, z = y
        dydt = [ diffEq_func[i](x, y, z, mu, h) for i in range(len(diffEq_func))]
        return dydt
    
    solution_set = []
    for _, x0, y0, z0, mu0, h0 in init_conds:
        sol = solve_ivp(Eqn, t_span=[0, duration], t_eval=t_eval, y0=[x0, y0, z0], args=(mu0, h0), method='RK45')
...
def sample_generation(s_size, steps, t_eval, key):
    ...
    z_sample = jax.random.uniform(get_random_key(key+9999), minval=-0.5, maxval=0.5, shape=(s_size, ))
    mu_sample = jax.random.uniform(get_random_key(key+12), minval=0.3, maxval=0.9, shape=(s_size, ))
    h_sample = jax.random.uniform(get_random_key(key+122), minval=0.3, maxval=0.9, shape=(s_size, ))

    dataset = jnp.stack([x_sample, y_sample, z_sample, mu_sample, h_sample], axis=1)
...
def return_func_output(eqn_num, state, func):
    return jax.vmap(lambda x, y, z, mu, h: func[eqn_num](x, y, z, mu, h))(state[:, 0], state[:, 1], state[:, 2], state[:, 3], state[:, 4])
...
def create(plotpred: bool, savepred: bool, savemetrics: bool, name: str):
    ...
    if plot_prediction:
        hp.plot_creations_2d(t_eval, test_dataset, test_target, steps, name=f"{save_name}_tb", no_of_params=2, no_of_svariables=3, no_plot_figures=10, set=save_name, pred_batch=pred_batch)
    ...
    for v in sec:
        ...
        if plot_prediction:
            hp.plot_creations_2d(t[:s], td, tt, s, name=f"{save_name}_tb" + "_" + str(v) + "sec", no_of_params=2, no_of_svariables=3, no_plot_figures=10, set=save_name, pred_batch=pt)
