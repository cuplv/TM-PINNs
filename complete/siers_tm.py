...
def get_ode_functions(num_derivatives=3):
    ...
    s, e, i, r, mu, beta, sigma, omega, alpha, gamma = sp.symbols('s e i r mu beta sigma omega alpha gamma')
    N = s + e + i + r
    params, state_vars = [mu, beta, sigma, omega, alpha, gamma], [s, e, i, r]
    pendEqn = [ mu*(N-s) - beta*s*i/N + omega * r, beta*s*i/N - (sigma+mu)*e, sigma*e - (mu + gamma + alpha)*i, gamma*i - (mu+omega)*r ]

    F_dot = sp.Matrix([pendEqn[v] for v in range(len(pendEqn))])
    F_ddot = sp.Matrix([sp.diff(F_dot[v], s) * pendEqn[0] + sp.diff(F_dot[v], e) * pendEqn[1] + sp.diff(F_dot[v], i) * pendEqn[2] + sp.diff(F_dot[v], r) * pendEqn[3] for v in range(len(F_dot))])
    F_dddot = sp.Matrix([sp.diff(F_ddot[v], s) * pendEqn[0] + sp.diff(F_ddot[v], e) * pendEqn[1] + sp.diff(F_dot[v], i) * pendEqn[2] + sp.diff(F_dot[v], r) * pendEqn[3] for v in range(len(F_ddot))])
...
def define_ode(init_conds, t_eval, duration):
    ...
    s, e, i, r, mu, beta, sigma, omega, alpha, gamma = sp.symbols('s e i r mu beta sigma omega alpha gamma')
    N = s + e + i + r
    diffEq = [ mu * (N-s) - (beta * s * i)/N + omega  *  r, (beta * s * i)/N - (sigma+mu) * e, sigma * e - (mu + gamma + alpha) * i, gamma * i - (mu+omega) * r ]
    diffEq_func = [ sp.lambdify((s, e, i, r, mu, beta, sigma, omega, alpha, gamma), val) for val in diffEq ]

    def Eqn(t, y, mu, beta, sigma, omega, alpha, gamma):
        s, e, i, r= y
        dydt = [ diffEq_func[v](s, e, i, r, mu, beta, sigma, omega, alpha, gamma) for v in range(len(diffEq_func))]
        return dydt
    
    solution_set = []
    for _, s0, e0, i0, r0, mu0, beta0, sigma0, omega0, alpha0, gamma0 in init_conds:
        sol = solve_ivp(Eqn, t_span=[0, duration], t_eval=t_eval, y0=[s0, e0, i0, r0], args=(mu0, beta0, sigma0, omega0, alpha0, gamma0), method='RK45')
...
def sample_generation(s_size, steps, t_eval, key):
    ...
    x_sample = jax.random.uniform(get_random_key(key), minval=0, maxval=0.99, shape=(s_size, ))
    y_sample = jax.random.uniform(get_random_key(key+99), minval=0, maxval=0.99, shape=(s_size, ))
    z_sample = jax.random.uniform(get_random_key(key+9999), minval=0, maxval=0.5, shape=(s_size, )) # μ β sigma ω α γ  
    a_sample = jax.random.uniform(get_random_key(key+777), minval=0, maxval=0.5, shape=(s_size, )) # μ β σ ω α γ  
    mu_sample = jax.random.uniform(get_random_key(key+12), minval=1/100, maxval=1/50, shape=(s_size, ))
    beta_sample = jax.random.uniform(get_random_key(key+88), minval=0.1, maxval=0.3, shape=(s_size, ))
    sigma_sample = jax.random.uniform(get_random_key(key+777), minval=1/10, maxval=1/5, shape=(s_size, ))
    omega_sample = jax.random.uniform(get_random_key(key+777), minval=0.1, maxval=1, shape=(s_size, ))
    alpha_sample = jax.random.uniform(get_random_key(key+777), minval=0, maxval=0.5, shape=(s_size, ))
    gamma_sample = jax.random.uniform(get_random_key(key+8), minval=1/100, maxval=1/5, shape=(s_size, ))

    dataset = jnp.stack([x_sample, y_sample, z_sample, a_sample, mu_sample, beta_sample, sigma_sample, omega_sample, alpha_sample, gamma_sample], axis=1)
...
def return_func_output(eqn_num, state, func):
    return jax.vmap(lambda x, y, z, a, mu, beta, sigma, omega, alpha, gamma: func[eqn_num](x, y, z, a, mu, beta, sigma, omega, alpha, gamma))(state[:, 0], state[:, 1], state[:, 2], state[:, 3], state[:, 4], state[:, 5], state[:, 6], state[:, 7], state[:, 8], state[:, 9])
...
def create(plotpred: bool, savepred: bool, savemetrics: bool, name: str):
    ...
    model_a = return_nnmodel(hp, ran_key, num_features, num_targets, num_layers, num_neurons_per_layer)

    model = [model_x, model_y, model_z, model_a]
    ...
    for iter in range(1):
        ...
        adam_state = initialize_adam_state(model, num_var=4)  # Initialize Adam state
        batch_counter = 0
        epochs = 500
        lr = 5e-1
        ...
        for itrain in range(batch_counter, batch_counter+epochs):
            ...
            ps_tc_x, ps_tc_y, ps_tc_z, ps_tc_a, lps_tc_x, lps_tc_y, lps_tc_z, lps_tc_a = get_intr_time_coupled_sums(activations, steps, train_t, train_t2, train_t3)
            
            model, adam_state = adam_update_gd(model, adam_state, activations, ps_tc_x, ps_tc_y, ps_tc_z, ps_tc_a, lps_tc_x, lps_tc_y, lps_tc_z, lps_tc_a, train_t3, train_t4, lr, alpha=1.0, beta=1.0)
    ...
    ps_tc_x, ps_tc_y, ps_tc_z, ps_tc_a, _, _, _, _ = get_intr_time_coupled_sums(test_dataset, steps, test_t, test_t2, test_t3)
    ...
    test_prediction_a = jnp.ravel(batched_prediction(model[3], test_dataset, silu)) # validation_batch_size*shape x 1
    ...
    time_coupled_a = ps_tc_a + test_t4 * test_prediction_a

    pred_batch = np.stack([time_coupled_x, time_coupled_y, time_coupled_z, time_coupled_a], axis=1)
    ...
    if plot_prediction:
        hp.plot_creations_2d(t_eval, test_dataset, test_target, steps, name=f"{save_name}_tb", no_of_params=6, no_of_svariables=4, no_plot_figures=10, set=save_name, pred_batch=pred_batch)
        hp.plot_error_distributions(pred_batch, test_target, save_name,  name=f"{save_name}_tb", no_of_svariables=4)
    ...
    for v in sec:
        ...
        if plot_prediction:
            hp.plot_creations_2d(t[:s], td, tt, s, name=f"{save_name}_tb" + "_" + str(v) + "sec", no_of_params=6, no_of_svariables=4, no_plot_figures=10, set=save_name, pred_batch=pt)
            hp.plot_error_distributions(pt, tt, save_name,  name=f"{save_name}_tb" + "_" + str(v) + "sec", no_of_svariables=4)
